...
ankle_means = ankle[[c for c in ankle.columns if "mean" in c]]
ankle_acc = ankle[[c for c in ankle.columns if "acc" in c]]

chest = data[[c for c in data.columns if "chest" in c]]
chest_means = chest[[c for c in chest.columns if "mean" in c]]
chest_acc = chest[[c for c in chest.columns if "acc" in c]]

hand = data[[c for c in data.columns if "hand" in c]]
hand_means = hand[[c for c in hand.columns if "mean" in c]]
hand_acc = hand[[c for c in hand.columns if "acc" in c]]
...
def model(features, labels):
    ...
    scores = []
    for name, clf in [('GaussianNB', gnb)]:
...
ankle_means_results = model(ankle_means, labels)
ankle_acc_results = model(ankle_acc, labels)

chest_results = model(chest, labels)
chest_means_results = model(chest_means, labels)
chest_acc_results = model(chest_acc, labels)

hand_results = model(hand, labels)
hand_means_results = model(hand_means, labels)
hand_acc_results = model(hand_acc, labels)
